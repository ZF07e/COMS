<?php
    ...
    require ('../../LandingPge/Functions/SessionManagement.php');
    ...
    function updateAssociation(){
        ...
        if(!isset($_POST["submit"])){
            ...
            $updateQuery = "UPDATE associations SET mission = ?, vision = ?, description = ? WHERE association = $affiliation";

            $stmt = $mysqli->stmt_init();
            if(!$stmt->prepare($updateQuery)){
                die("SQL Error". $mysqli->error);
            }
            $stmt->bind_param("sss", $mission, $vision, $description);
            $stmt->execute();
            $mysqli->close();
